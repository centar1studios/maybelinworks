...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const projectGrid = document.getElementById("featured");
    ...
    function getPanelTarget(button) {
        if (button.dataset.panelOpen) {
            return button.dataset.panelOpen;
        }

        if (button.dataset.dialogOpen === "dialog_works") {
            return "panel_works";
        }

        if (button.dataset.dialogOpen === "dialog_about") {
            return "panel_about";
        }
    ...
    function getPanelHash(panelId) {
        if (panelId === "panel_works") {
            return "#works";
        }

        if (panelId === "panel_about") {
            return "#about";
        }
    ...
    function focusPanel(panel) {
        ...
        let focusTarget = titleId
            ? document.getElementById(titleId)
            : null;

        if (!focusTarget) {
            focusTarget = panel.querySelector("h1, h2, h3");
        }
    ...
    function openPanel(panelId, options = {}) {
        ...
        if (updateHistory) {
            const hash = getPanelHash(panelId);
    ...
    function applySettings(settings) {
        ...
        const aboutPhoto = document.getElementById("about_photo");

        if (aboutPhoto && settings.about_photo_url) {
            aboutPhoto.src = settings.about_photo_url;
            aboutPhoto.alt = settings.about_photo_alt || "";
        }
        ...
        if (heroVideo && settings.hero_video_url) {
        ...
        if (footerBrand) {
            ...
            footerBrand.appendChild(yearSpan);
    ...
    function createProjectCard(project) {
        ...
        number.textContent =
            `PROJECT ${String(project.project_number).padStart(2, "0")}`;
